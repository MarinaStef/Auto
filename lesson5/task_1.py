...
try:
    ...
    for _ in range(5):
        ...
        firefox_delete_buttons = firefox.find_element(
            "xpath", '//button[text()="Delete"]')
    
    print(
        f"Размер списка Delete в Chrome: {len(chrome_delete_buttons)}")
    print(
        f"Размер списка Delete в Firefox: {len(firefox_delete_buttons)}")
